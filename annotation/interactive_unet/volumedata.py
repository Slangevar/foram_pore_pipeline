...
class VolumeData(object):
    ...
    def __init__(self, file, annotations=False):
        ...
        if annotations:
            ...
            self.candidates, self.class_weights = self.slicer.get_origin_candidates(
                self.mask_volume
            )
    ...
    def build_annotation_volumes(self):
        ...
        mask_volume = np.zeros(
            (
                self.image_volume.shape[0],
                self.image_volume.shape[1],
                self.image_volume.shape[2],
            ),
            dtype="uint8",
        )
        weight_volume = np.zeros(
            (
                self.image_volume.shape[0],
                self.image_volume.shape[1],
                self.image_volume.shape[2],
        ...
        for i in range(len(slice_files)):
            ...
            if slice_data["volume"] == self.filename:
                ...
                # Load sample
                mask = io.imread(
                    slice_files[i].replace("slices", "masks").replace(".npy", ".tiff")
                )
                weight_train = io.imread(
                    slice_files[i].replace("slices", "weights").replace(".npy", ".tiff")
                )
                weight_val = io.imread(
                    slice_files[i]
                    .replace("slices", "weights")
                    .replace(".npy", ".tiff")
                    .replace("train", "val")
                )
                ...
                weight_volume[..., 0] = self.slicer.update_volume(
                    weight_train, weight_volume[..., 0]
                )
                weight_volume[..., 1] = self.slicer.update_volume(
                    weight_val, weight_volume[..., 1]
                )
    ...
    def sample(
        ...
        image_slice = self.slicer.get_slice(
            self.image_volume, slice_width=slice_width, order=order
        )
        mask_slice = self.slicer.get_slice(
            self.mask_volume, slice_width=slice_width, order=0
        )
        weight_slice = self.slicer.get_slice(
            self.weight_volume[..., weight_channel], slice_width=slice_width, order=0
        )
