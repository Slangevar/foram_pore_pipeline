...
class Annotator(object):
    ...
    def update_display(self, annotation_opacity=0.25):
        ...
        if annotation_opacity > 0:
            ...
            image[mask_overlay_region] = (
                image[mask_overlay_region] * (1 - annotation_opacity)
                + mask[mask_overlay_region] * annotation_opacity
            )
    ...
    def get_roi_image(self, size=None):
        ...
        if size is None:
            x_grid, y_grid = np.meshgrid(
                np.linspace(self.roi[0], self.roi[2], self.canvas_size)
                * (self.display_image.shape[0] - 1),
                np.linspace(self.roi[1], self.roi[3], self.canvas_size)
                * (self.display_image.shape[1] - 1),
            )
        else:
            x_grid, y_grid = np.meshgrid(
                np.linspace(self.roi[0], self.roi[2], size)
                * (self.display_image.shape[0] - 1),
                np.linspace(self.roi[1], self.roi[3], size)
                * (self.display_image.shape[1] - 1),
            )

        if len(self.display_image.shape) > 2:
            roi_image = np.concatenate(
                [
                    map_coordinates(
                        self.display_image[:, :, i], np.array([y_grid, x_grid]), order=0
                    )[:, :, None]
                    for i in range(self.display_image.shape[2])
                ],
                axis=2,
            )
        else:
            roi_image = map_coordinates(
                self.display_image, np.array([y_grid, x_grid]), order=0
            )
